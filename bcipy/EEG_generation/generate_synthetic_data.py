...
def generate_synthetic_data(tau, theta, Cmat, linear_model_weights, stimulis, x0):
    # Time vector
    t = np.arange(0, 396)
    ...
    for i in range(len(stimulis)):
        input_signal = stimulis[i]  # Ensure this is your squared signal input of size (3, 792)
        x = odeint(model_x, x0, t, args=(tau, Cmat, theta, input_signal[0]))
