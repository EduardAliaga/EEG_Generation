...
def analyze_erp(erp_data, parameters, device_spec, data_folder, estimate_balanced_acc,
    ...
    for i in range(inquiries.shape[0]):
        # Create a DataFrame from the slice of the numpy array
        df = pd.DataFrame(inquiries[i].T)  # Transpose to have 100 rows and 792 columns
        # Save the DataFrame to a CSV file
        df.to_csv(f'/Users/aliag/Desktop/BciPy/bcipy/EEG_generation/data_{i}.csv', index=False)

    inquiry_timing = update_inquiry_timing(inquiry_timing, downsample_rate)
    inquiry_timing = np.array(inquiry_timing)
    dt = pd.DataFrame(inquiry_timing)
    inquiry_labels = np.array(inquiry_labels)
    labels = pd.DataFrame(inquiry_labels)
    # Save the DataFrame to a CSV file
    dt.to_csv(f'/Users/aliag/Desktop/BciPy/bcipy/EEG_generation/timing.csv', index=False)
    labels.to_csv(f'/Users/aliag/Desktop/BciPy/bcipy/EEG_generation/labels.csv', index=False)
