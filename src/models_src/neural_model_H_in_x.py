...
def compute_states_mse(states_predicted, states_real):
        training_mse_states = np.mean((states_predicted[0:2500,0,:] - states_real[0,:,:2500].T) ** 2)
        testing_mse_states = np.mean((states_predicted[:2500,0,:] - states_real[0,:,:2500].T) ** 2)
    ...
    def fit(self, stimuli, measurements_noisy, states): 
        ...
        for i in tqdm(range(0,100)):
            ...
            for t in tqdm(range(num_time_points)):
                ...
                self.H = self.x[9:11].reshape(2,2)
