...
def jacobian_f_o(x, u, W, M, tau, dt):
    F_W = np.array(jax.jit(jax.jacobian(f_o,argnums=(2)))(x, u, W, M, tau, dt)).reshape(2,4)
    F_M = np.array(jax.jit(jax.jacobian(f_o,argnums=(3)))(x, u, W, M, tau, dt)).reshape(2,1)
    F_tau = np.array(jax.jit(jax.jacobian(f_o,argnums=(4)))(x, u, W, M, tau, dt)).reshape(2,1)
    J_combined = np.hstack((F_W, F_M, F_tau))
    return np.array(J_combined)
...
def recursive_update (x, theta, W, M, H, tau, u, y, dt, P_x_, P_x, Q_x, P_params_, P_params, Q_params,R_y, num_iterations):
    ...
    for t in range(1, num_iterations):
        ...
        F_x = np.array(jax.jit(jax.jacobian(f_o,argnums=(0)))(x, u[t-1], W, M, tau, dt))
        F_params = jacobian_f_o(x, u[t-1], W, M, tau, dt)
